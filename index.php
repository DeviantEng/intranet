...
<html>
	<head>
		<title><?php echo $config['wifiName']; ?> Intranet</title>
		<link rel="stylesheet" href="intranet/style.css" />
		<link rel="shortcut icon" href="favicon.ico" />
	</head>
	<body>
		<h1><?php echo $config['wifiName']; ?> Server</h1>
		...
		<?php ## Action Buttons ?>
		<?php if( $config['sickbeard'] ) : ?>
		<a href="<?php echo $sickbeardURL; ?>" title="SickBeard" class="actionButton big sickbeard" target="_blank"><span>SickBeard</span></a>
		<?php endif; ?>
		<?php if( $config['couchpotato'] ) : ?>
		<a href="<?php echo $couchpotatoURL; ?>" title="CouchPoato" class="actionButton big couchpotato" target="_blank"><span>CouchPotato</span></a>
		<?php endif; ?>
		<?php if( $config['headphones'] ) : ?>
		<a href="<?php echo $headphonesURL; ?>" title="Headphones" class="actionButton big headphones" target="_blank"><span>Headphones</span></a>
		<?php endif; ?>

		<?php ## SABnzbd ?>
		<?php if( $config['sabnzbd'] ) : ?>
		<a href="<?php echo $sabURL; ?>" title="SABnzbd" class="actionButton big sabnzb" target="_blank"><span>SABnzbd</span></a>
		...
		<section class="clearfix">
			<a href="http://<?php echo $config['uTorrentURL']; ?>:<?php echo $config['uTorrentPort']; ?>/gui/" title="uTorrent" class="actionButton big utorrent" target="_blank"><span>uTorrent</span></a>
...
$rpc->url = $transmissionURL."/transmission/rpc";
		...
		<?php if( $config['showWifi'] ) : ?>
			<div class="wifi">
				<h2>Wifi Password for <?php echo $config['wifiName'] ?></h2>
				<big><?php echo $config['wifiPassword']; ?></big>
			</div>
    ...
    <div class="uptime">
		<?php $rebootresult = mysqli_query($observiumcon,"$rebootedquery");
      echo "<h2>Server Uptime Less Than 24H</h2>";
      echo "<table>
      <tr>
      <th>Hostname</th>
      <th>Type</th>
      <th>OS</th>
      <th>Uptime</th>
      <th>Last SNMP Polled</th>
      </tr>";

      while($row = mysqli_fetch_array($rebootresult))
        {
        echo "<tr>";
        ...
        echo "<td>" . $row['Last SNMP Poll'] . "</td>";
        echo "</tr>";
        }
      echo "</table>";

      mysqli_close($observiumcon); ?>	
      </div>
